...
BOOL CSortListCtrl::SetHeadings( const CString& strHeadings )
{
	...
	for( ;; )
	{
		...
		const int iWidth = atoi( (CStringA)strHeadings.Mid( iStart, iSemiColon - iStart ) );
...
{
	const int iIndex = InsertItem(GetItemCount(), pszText, GetItemCount());
...
int CSortListCtrl::NumberCompare( LPCTSTR pszNumber1, LPCTSTR pszNumber2 )
{
	...
	const int iNumber1 = atoi((CStringA)pszNumber1);
	const int iNumber2 = atoi((CStringA)pszNumber2);
...
{
	const int iYear1 = atoi((CStringA)strDate1.Mid(6, 4));
	const int iYear2 = atoi((CStringA)strDate2.Mid(6, 4));
	...
	const int iMonth1 = atoi((CStringA)strDate1.Mid(3, 2));
	const int iMonth2 = atoi((CStringA)strDate2.Mid(3, 2));
	...
	const int iDay1 = atoi((CStringA)strDate1.Mid(0, 2));
	const int iDay2 = atoi((CStringA)strDate2.Mid(0, 2));
...
int CALLBACK CSortListCtrl::CompareFunction( LPARAM lParam1, LPARAM lParam2, LPARAM lParamData )
{
	...
	if(pListCtrl->IsNumber( pszText1 ) )
		return pListCtrl->m_bSortAscending ? pListCtrl->NumberCompare( pszText1, pszText2 ) : pListCtrl->NumberCompare( pszText2, pszText1 );
	else if(pListCtrl->IsDate( pszText1 ) )
		return pListCtrl->m_bSortAscending ? pListCtrl->DateCompare( pszText1, pszText2 ) : pListCtrl->DateCompare( pszText2, pszText1 );
	else
		// text.
...
void CSortListCtrl::LoadColumnInfo()
{
	...
	UINT nBytes = 0;
	BYTE* buf = NULL;
	if (AfxGetApp()->GetProfileBinary(_T("ListCtrls"), strKey, &buf, &nBytes))
	{
		if( nBytes > 0 )
		{
...
void CSortListCtrl::SaveColumnInfo()
{
	...
	VERIFY(AfxGetApp()->WriteProfileBinary(_T("ListCtrls"), strKey, buf, dwLen));
