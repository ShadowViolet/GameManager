...
void CDlgBase::OnDropFiles(HDROP hDropInfo)
{
	...
	{		
		...
		if (0 == strFileName.CompareNoCase(TEXT("bmp")) || 0 == strFileName.CompareNoCase(TEXT("jpg")) || 0 == strFileName.CompareNoCase(TEXT("png")))
		{
			...
			{
				if (sqlite3_exec(theApp.db, "INSERT INTO 管家设置 VALUES( '基本设置', '管家皮肤','" + (CStringA)FileName + "')", NULL, NULL, &theApp.errMsg) != SQLITE_OK)
				{
			...
			{
				if (sqlite3_exec(theApp.db, "UPDATE 管家设置 SET Value = '" + (CStringA)FileName + "' WHERE Name = '管家皮肤' and  Type = '基本设置' ", NULL, NULL, &theApp.errMsg) != SQLITE_OK)
				{
					MessageBox((CString)theApp.errMsg);
					return;
				}
